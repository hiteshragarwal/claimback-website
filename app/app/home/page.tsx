...
export default function HomePage() {
  ...
  const activeCases = cases.filter(c => !['won', 'lost'].includes(c.outcome_type as string));
      ...
      <Link href="/upload" className="block mb-6">
        <div className="bg-[#06195e] rounded-2xl p-5 text-white shadow-lg active:scale-[0.98] transition-transform">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs bg-white/20 px-2.5 py-1 rounded-full">₹1,999</span>
          </div>
      ...
      {cases.length === 0 && !loading && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6">
          ...
          {[
            ...
            { n: '2', t: 'Pay ₹1,999 flat fee',   d: 'No hidden charges. ₹799 refund if we can\'t help.' },
        ...
        <div>
          ...
          <div className="space-y-2">
            {cases.slice(0, 3).map(c => {
              ...
              return (
                <Link key={id} href={status === 'analysed' ? `/results?case=${id}` : `/cases/${id}`}>
                  <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center justify-between active:bg-gray-50">
                    <div>
                      <p className="font-medium text-sm text-gray-900">{(c.insurer_name as string) || 'Draft case'}</p>
